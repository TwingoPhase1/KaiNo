...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col justify-between pb-6">
      <div className="max-w-4xl mx-auto p-4 w-full flex-grow">
        <PlatformHeader
          title="Kaino"
          extra={<SyncIndicator compact hideCount />}
          actions={
            <div className="flex items-center gap-2">
              {currentUser && (
                <span className="text-xs text-muted-foreground mr-2 font-medium bg-muted dark:bg-[#1c1c1e] px-2.5 py-1 rounded-full flex items-center gap-1.5 border border-slate-200/50 dark:border-white/5">
          ...
          <div className="flex gap-3 w-full items-center mb-6">
            ...
            {theme !== 'theme-android' && (
              <Button
                ...
                variant={theme === 'theme-ios' ? 'ghost' : 'default'}
                className={
                  theme === 'theme-ios'
                    ? 'ios-text-btn font-semibold text-primary px-2 h-auto text-[17px] hover:bg-transparent'
                    : theme === 'theme-samsung'
                    ? 'rounded-[20px] bg-indigo-600 text-white hover:bg-indigo-500 shadow-md shadow-indigo-600/10 px-5 py-3 h-auto text-sm'
                    : 'theme-btn font-semibold gap-1.5 shrink-0 rounded-md'
                }
              >
                {theme !== 'theme-ios' && <Plus className="h-4 w-4" />}
                <span>{theme === 'theme-ios' ? "Ajouter" : (t('dashboard_lists_create_btn') || "Créer")}</span>
              </Button>
            )}
          </div>
                ...
                <div className="ios-inset-grouped">
                  {filteredLists.map((list) => (
                    <div
                      ...
                      className="group flex items-center justify-between p-4 bg-white/95 dark:bg-[#1c1c1e]/90 backdrop-blur-md hover:bg-slate-50 dark:hover:bg-[#2c2c2e]/90 active:opacity-75 transition-all cursor-pointer select-none"
                    >
                  ...
                  if (theme === 'theme-samsung') {
                    return (
                      <div
                        key={list.id}
                        onClick={() => router.push(`/lists/${list.id}`)}
                        className="oneui-card group flex items-center justify-between p-5 hover:shadow-[0_12px_32px_rgba(0,0,0,0.04)] active:scale-[0.98] transition-all duration-200 cursor-pointer select-none border border-slate-100/10"
                      >
      ...
      <PlatformFab onClick={() => router.push('/lists/new')} label="Créer une liste" />
    </div>
  );
}
